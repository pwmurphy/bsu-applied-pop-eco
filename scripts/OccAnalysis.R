...
datadir <- paste( getwd(), "/data/class_examples/", sep = "" )
...
# Answer: 
# because the temperature variables are the same for all sites each year so in one year, 
# the value is the same for every site
...
umf <- unmarkedFrameOccu( y = as.matrix( closeddf[ ,c("pres.j1", "pres.j2", "pres.j3")]),
      ...
      siteCovs = closeddf[ ,c("sagebrush", "cheatgrass", "Feb.minT", "AprMay.maxT")],
...
# Answer:
# If we scale them, they are all on the same scale and the magnitude of the effects
# can be compared directly between predictors

# View summary of scaled unmarked dataframe:
summary( umf )
# What does it tell us?
# Answer:
# it tells us about the distribution of values for our predictors and tells us about 
# how many surveys each technician did
...
# Answer: because the coefficients refer to the slopes/magnitude of effect of the predictor on 
# the logit scale 
...
# Answer: using the confidence intervals for the coefficients, predictors where the confidence intervals do not include 0
# are important for the response variables. For the ecological submodel, sagebrush appears to have a positive effect on pr(occupancy) and for the 
# detection submodel, there is some effect of observer-- observer 2, 3, and 4 seem to have lower detection compared to observer 1 (intercept)
...
# Answer:
# when you have too many variables of interest for your sample size so you cannot include them all in your model
# or when you have sets of highly correlated variables that can't be included in a single model together but you are interested
# in including one of them and want to see if one has more support than others
...
unmarked::vif(fm.closed, type = "state")
unmarked::vif(fm.closed, type = "det")
...
# Answer: values between 1 and 1.2
#
# Which values would have been a concern? 
# Answer: values above 5 but non are concerning
#
...
# Answer: because we cannot directly compare these models if we are also changing the detection submodel
# and only one of the veg predictors in the detection model so it shouldn't be an issue
...
# Answer: the best supported model was the model with sagebrush (AIC 203.74 and AIC wt of 0.96) 
# and the cheatgrass model had a delta AIC of 6.12 suggested the sagebrush model is more supported by the data
# the null had a deltaAIC of 33.62 suggested our parametized models were much better supported
...
# Answer: the full model estimates occupancy at more sites than the naive occupancy; the sagebrush model predicts similar values to the full model with a few exceptions
# and the cheatgrass model estimates occupancy at less sites than the full 
...
# Answer: estimated occupancy was 0.643
#
...
# Answer: occupancy estimates are higher for observer 1 than any other (especially over observer 4) and sagebrush has a negative impact 
...
# Answer:
# sometimes it takes a long time to run (esp Bayesian) and can open up room for mistakes if we accidently re run lines of code out of order or change underlying file locations
